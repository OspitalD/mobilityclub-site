...
test('la série à 249 € expose quatorze tickets pris et six tickets libres', async () => {
  captureBlobs = [];
  ...
  assert.equal(payload.sold, 34);
  assert.equal(payload.remaining, 6);
  assert.equal(payload.tier, 2);
  assert.equal(payload.tickets.length, 20);
  assert.deepEqual(payload.tickets.filter((ticket) => ticket.status === 'sold').map((ticket) => ticket.number), [1, 4, 5, 6, 7, 8, 10, 12, 13, 14, 15, 16, 18, 20]);
  assert.deepEqual(payload.tickets.filter((ticket) => ticket.status === 'available').map((ticket) => ticket.number), [2, 3, 9, 11, 17, 19]);
});

test('la quinzième capture PayPal ajoute un ticket vendu au motif déclaré', async () => {
  captureBlobs = Array.from({ length: 15 }, (_, index) => `249/CAP-${index + 1}`);
  const response = await handler(new Request('http://localhost/.netlify/functions/lifetime-tickets'));
  const payload = await response.json();
