...
const lireCaptures = async () => {
  ...
  return {
    ...
    1: captures.filter((capture) => capture.key.startsWith('199/') && sansNumero(capture)).length,
    2: captures.filter((capture) => capture.key.startsWith('249/') && sansNumero(capture)).length,
  };
};
